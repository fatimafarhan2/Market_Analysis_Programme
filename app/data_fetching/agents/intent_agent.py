...
"""
        try:
            ...
            products = []
            countries = []
            if "product_name_contains" in parsed["filters"]:
                EXTRACTED_PRODUCTS = [p.strip() for p in parsed["filters"]["product_name_contains"].split(",") if p.strip()]

            if "country_name" in parsed["filters"]:
                EXTRACTED_COUNTRIES = [c.strip() for c in parsed["filters"]["country_name"].split(",") if c.strip()]
