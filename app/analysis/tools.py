...
@tool
def swot_analysis(df_json: str, pivot_brand: str) -> Dict[str, List[str]]:
    """
    ...
    ----------
    df_JSON : json string
        json containing product- and market-level information for multiple brands.
        Must include key columns such as:
        - 'brand_name', 'total_revenue_usd', 'avg_rating', 'avg_sentiment_score',
          'avg_marketing_efficiency_ratio', 'avg_return_on_marketing_spend',
          'avg_online_sales_ratio', 'avg_returns_rate', 'price_range', 
          'avg_income_usd', 'region', 'country_name', etc.
        
        These features are grouped internally to evaluate internal (strengths/weaknesses)
        and external (opportunities/threats) aspects of the pivot brand.

    pivot_brand : str
        The name of the brand to evaluate. This brand will be compared against all 
        other brands (competitors) in the DataFrame.
